...
manifests: controller-gen ## Generate WebhookConfiguration, ClusterRole and CustomResourceDefinition objects.
	$(CONTROLLER_GEN) $(CRD_OPTIONS) rbac:roleName=multicluster-engine-operator-role paths="./..."  output:crd:dir=config/crd/bases
...
test: manifests generate fmt vet envtest ## Run tests (with coverage).
	KUBEBUILDER_ASSETS="$(shell $(ENVTEST) use $(ENVTEST_K8S_VERSION) --use-deprecated-gcs=false -p path)" DIRECTORY_OVERRIDE="../" UNIT_TEST=true go test $(shell go list ./... | grep -E -v "test") -coverprofile cover.out
