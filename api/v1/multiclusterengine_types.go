...
type NetworkPoliciesConfig struct {
	// Enabled controls whether NetworkPolicies are deployed for MCE components
	// Default: true in MCE 5.0+
...
type InternalEngineComponentSpec struct {
	...
	// upgrades. Resources are only removed if they still carry the backplaneconfig.name ownership
	// label applied by this operator (see utils.AddBackplaneConfigLabels), so manually recreated
